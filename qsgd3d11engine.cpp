...
static void getHardwareAdapter(IDXGIFactory1 *factory, IDXGIAdapter1 **outAdapter)
{
    ...
    if (qEnvironmentVariableIsSet("QT_D3D_ADAPTER_INDEX")) {
        ...
        if (SUCCEEDED(factory->EnumAdapters1(adapterIndex, &adapter))) {
            adapter->GetDesc1(&desc);
            const QString name = QString::fromUtf16((char16_t *) desc.Description);
            HRESULT hr = D3D11CreateDevice(adapter.Get(), D3D_DRIVER_TYPE_UNKNOWN, nullptr, 0, &fl, 1, D3D11_SDK_VERSION, nullptr, nullptr, nullptr);
            if (FAILED(hr)) {
    ...
    for (int adapterIndex = 0; factory->EnumAdapters1(adapterIndex, &adapter) != DXGI_ERROR_NOT_FOUND; ++adapterIndex) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            const QString name = QString::fromUtf16((char16_t *) desc.Description);
            qCDebug(QSG_LOG_INFO_GENERAL, "Failed to create D3D11 device on adapter '%s': %s", qPrintable(name), qPrintable(comErrorMessage(hr)));
        }
    }
...
ID3D11Device *QSGD3D11DeviceManager::ref()
{
    ensureCreated();
    m_ref.ref();
    return m_device.Get();
...
ID3D11DeviceContext1 *QSGD3D11DeviceManager::context()
{
    ensureCreated();
    return m_context.Get();
}

void QSGD3D11DeviceManager::unref()
{
    if (!m_ref.deref()) {
...
IDXGIFactory2 *QSGD3D11DeviceManager::dxgi()
{
    ensureCreated();
    return m_factory.Get();
}

void QSGD3D11DeviceManager::ensureCreated()
{
    if (m_device)
        return;
    ...
    if (d3dDebugLayer()) {
        ...
        qCDebug(QSG_LOG_INFO_GENERAL, "Enabling D3D debug layer");
    }
    ...
    bool warp = true;
    if (adapter) {
        HRESULT hr = D3D11CreateDevice(adapter.Get(), D3D_DRIVER_TYPE_UNKNOWN, nullptr, flags, &fl, 1, D3D11_SDK_VERSION, &m_device, nullptr, &context);
        if (SUCCEEDED(hr))
            warp = false;
        else
    ...
    if (warp) {
        ...
        HRESULT hr = D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_WARP, nullptr, flags, &fl, 1, D3D11_SDK_VERSION, &m_device, nullptr, &context);
        if (FAILED(hr)) {
    ...
    if (context) {
        HRESULT hr = context.CopyTo(IID_PPV_ARGS(&m_context));
        if (FAILED(hr)) {
            qWarning("Failed to get ID3DDeviceContext1: %s", qPrintable(comErrorMessage(hr)));
            return;
        }
    }
...
bool QSGD3D11Engine::attachToWindow(WId window, const QSize &size, float dpr, int surfaceFormatSamples, bool alpha)
{
    if (d->isInitialized()) {
...
WId QSGD3D11Engine::window() const
{
...
void QSGD3D11EnginePrivate::initialize(WId w, const QSize &size, float dpr, int surfaceFormatSamples, bool alpha)
{
    if (initialized)
        return;
    ...
    QSGD3D11DeviceManager *dev = deviceManager();
    device = dev->ref();
    context = dev->context();
    ...
    if (windowAlpha) {
        ...
        HRESULT hr = dev->dxgi()->CreateSwapChainForComposition(device, &swapChainDesc, nullptr, &baseSwapChain);
        if (SUCCEEDED(hr)) {
    ...
    if (!windowAlpha) {
        ...
        swapChainDesc.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
        swapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD; // D3D11 requires the flip model
        ...
        HRESULT hr = dev->dxgi()->CreateSwapChain(device, &swapChainDesc, &baseSwapChain);
        if (FAILED(hr)) {
    ...
    if (waitableSwapChainMaxLatency) {
        ...
        if (SUCCEEDED(swapChain.CopyTo(IID_PPV_ARGS(&swapChain2)))) {
            if (FAILED(swapChain2->SetMaximumFrameLatency(waitableSwapChainMaxLatency)))
                qWarning("Failed to set maximum frame latency to %d", waitableSwapChainMaxLatency);
            swapEvent = swapChain2->GetFrameLatencyWaitableObject();
        }
    }
...
{
    if (FAILED(swapChain->GetBuffer(0, IID_PPV_ARGS(&backBufferRT)))) {
        qWarning("Failed to get backbuffer from swap chain");
        return;
    }

    if (windowSamples == 1) {
        defaultRT = backBufferRT;
        device->CreateRenderTargetView(defaultRT.Get(), nullptr, &defaultRTV);
    } else {
        const QSize size(windowSize.width() * windowDpr, windowSize.height() * windowDpr);
        auto msaaRT = createColorBuffer(size, windowSamples);
        defaultRT = msaaRT.first;
        defaultRTV = msaaRT.second;
    }

    const QSize size(windowSize.width() * windowDpr, windowSize.height() * windowDpr);
    auto ds = createDepthStencil(size, windowSamples);
    defaultDS = ds.first;
    defaultDSV = ds.second;
...
void QSGD3D11EnginePrivate::beginFrameDraw()
{
    ...
    /*if (windowSamples == 1)
        transitionResource(defaultRT[presentFrameIndex % swapChainBufferCount].Get(), commandList,
               D3D11_RESOURCE_STATE_PRESENT, D3D11_RESOURCE_STATE_RENDER_TARGET);*/
}
...
void QSGD3D11EnginePrivate::endDrawCalls(bool lastInFrame)
{
    ...
    if (lastInFrame) {
        ...
        // Resolve multisampled render target if we're not rendering to the back buffer directly
        const int idx = presentFrameIndex % swapChainBufferCount;
        if (windowSamples != 1) {
            if (Q_UNLIKELY(debug_render())) {
                D3D11_TEXTURE2D_DESC desc;
                defaultRT->GetDesc(&desc);
                qDebug("added resolve for multisampled render target (count %d, quality %d)",
                       desc.SampleDesc.Count, desc.SampleDesc.Quality);
            }
            resolveMultisampledTarget(defaultRT.Get(), backBufferRT.Get());
        }
...
void QSGD3D11EnginePrivate::queueSetRenderTarget(uint id)
{
    ...
    if (!id) {
        rtvHandle = defaultRTV.Get();
        dsvHandle = defaultDSV.Get();
    } else {
        const int idx = id - 1;
...
void QSGD3D11EnginePrivate::queueClearRenderTarget(const QColor &color)
{
    ...
    auto rtv = !currentRenderTarget
            ? defaultRTV.Get()
            : renderTargets[currentRenderTarget - 1].rtv.Get();
...
void QSGD3D11EnginePrivate::queueClearDepthStencil(float depthValue, quint8 stencilValue, QSGD3D11Engine::ClearFlags which)
{
    ...
    auto dsv = !currentRenderTarget
            ? defaultDSV.Get()
            : renderTargets[currentRenderTarget - 1].dsv.Get();
